...
class ProjectsController < ApplicationController
  ...
  #unique task  list return  
  def uniqueTaskListGeneration
    pta = []
  ...
  #unique issue list return
  def uniqueIssueListGeneration
    pti = []
    issueSet = Issue.where(project_id: @project.id, tracker_id: 5)
    issueSet.each do |individualIssue|
      if individualIssue.parent_id?
        pti<< individualIssue.parent_id
      end
    end
    issueSet.reject{|s| pti.uniq.include? s.id}
    return issueSet
  end

  #done percentage pdtd calculation
  def PDTD_calculation(start_date,taskSet)
        @day_Doneper= []
        @practical_daynumber= []        
        
        kk=0
        while kk<=@number_of_day
          @day_Doneper[kk]= 0
          @practical_daynumber[kk]=0
          kk += 1
        end 
        ...
        v=0
        taskSet.each do |t|
          issueNo=Issue.find(t.id)
          idNo= issueNo.id
          #Rails.logger.info idNo
          rank = issueNo.priority_id
          taskIDtrack[v]=idNo
          rankID[v]=rank
          v+=1
        end      
#=begin
        arraySet= Array.new(@number_of_day+1){Array.new(taskIDtrack.count,0)}
        taskSet.each do |t|
          issueNo   = Issue.find(t.id)
          j_detail  = issueNo.journals.collect {|j| j.details}.flatten
          resultSet = j_detail.collect {|k| [k.prop_key, k.old_value, k.value, k.journal.created_on,k.journal.journalized_id]}
          idNumber  = taskIDtrack.index(issueNo.id)

          resultSet.each{|r|
            if (r[0]=="done_ratio")
               #Rails.logger.info r
               cur_date=DateTime.strptime(r[3].to_s,'%Y-%m-%d')
               day_number=projectDuration(start_date, cur_date)
               if(day_number<=@number_of_day)
                 arraySet[day_number][idNumber]= r[2].to_i
               end             
            end
          }
        end
        ...
        while kk<= @number_of_day
          jj=0
          while jj< taskIDtrack.count
            if (kk != 0 && arraySet[kk][jj] == 0)
              arraySet[kk][jj] = arraySet[kk-1][jj]
            end
            jj += 1
          end
          kk += 1
        ...
        while day<=@number_of_day
          prio=0
          val=0
          while prio < taskIDtrack.count
              val += (arraySet[day][prio]*rankID[prio]*0.01)
              prio += 1
          end
          @day_Doneper[day] = (1.0-(@point-val)/@point)*100.0 
          day+=1
        end
  end

  #done percentage pdtd calculation
  def PDTD_forIssueCalculation(start_date,taskSet)
        @issue_day_Doneper=[]
        @issue_practical_daynumber=[]
        kk=0
        while kk<=@number_of_day
          @issue_day_Doneper[kk]= 0
          @issue_practical_daynumber[kk]=0
          kk += 1
        end 
        taskIDtrack=[]
        rankID=[]
        ...
        while day<=@number_of_day
          ...
          @issue_day_Doneper[day] = (1.0-(@point-val)/@point)*100.0 
          day+=1
        end
        #END OF PUT VALUE TO JAVASCRIPT KPI SHOW
  end
  ...
  def show
    ...
    if params[:jump] && redirect_to_project_menu_item(@project, params[:jump])
      return
       ...
       start_date = Date.parse(@project.created_on.to_s)
       end_dates  = CustomValue.where(customized_id: @project.id, custom_field_id: 1).pluck(:value)
       end_date   = Date.parse(end_dates[0])
       @number_of_day = projectDuration(start_date,end_date)              #X
       @number_of_day_inHour= @number_of_day*8;

       @projectTask_Daypass = projectDuration(start_date,Date.today)
       if(@projectTask_Daypass >= @number_of_day)
        @projectTask_Daypass = @number_of_day
       end
      
       getWeight = []
       getWeight = projectTaskWeightcalculation
       @weight= getWeight[0]
       @point = getWeight[1]
       taskSet=uniqueTaskListGeneration
       PDTD_calculation(start_date,taskSet)
       
       getWeight=[]
       getWeight=projectIssueWeightcalculation
       @issueWeight=getWeight[0]
       @issue_point=getWeight[1]        
       issueSet=uniqueIssueListGeneration
       PDTD_forIssueCalculation(start_date,issueSet)
